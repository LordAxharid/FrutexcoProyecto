...
class NewsletterController extends Controller
{
    ...
    public function addSubscriber(Request $request){
        if($request->ajax()){
            ...
            if($subscriberCount>0){
                ...
                // Agregar Correo A EL Boletin informativo En La Tabla newsletter_subscribers
    ...
    public function updateNewsletterStatus($id,$status){
        ...
        return redirect()->back()->with('flash_message_success','Suscripcion a noticias actualizada correctamente!');
    }

    public function deleteNewsletterEmail($id){
        NewsletterSubscriber::where('id',$id)->delete();
        return redirect()->back()->with('flash_message_success','El email de suscripcion a noticiais ha sido eliminada correctamente!');
    }
